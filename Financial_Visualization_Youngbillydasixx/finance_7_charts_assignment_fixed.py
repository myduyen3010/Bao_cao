...
plt.legend(handles=[
    Patch(color="#2ca02c", label="Xanh lá: Thu nhập"),
    Patch(color="#d62728", label="Đỏ: Chi tiêu"),
    Patch(color="#1f77b4", label="Xanh dương: Dòng tiền ròng"),
], loc="upper right")
...
plt.barh(budget_compare["category_norm"], budget_compare["variance"],
         color=np.where(budget_compare["variance"] >= 0, "#2ca02c", "#d62728"))
plt.axvline(0, color="black", linewidth=1)
plt.title("2. BIÊN ĐỘ LỆCH RÒNG GIỮA THỰC CHI VỚI HẠN MỨC NGÂN SÁCH", fontweight="bold")
plt.xlabel("Ngân sách - Thực chi")
plt.ylabel("Danh mục")
plt.legend(handles=[
    Patch(color="#2ca02c", label="Xanh lá: Chi dưới ngân sách"),
    Patch(color="#d62728", label="Đỏ: Vượt ngân sách"),
], loc="lower right")
...
for bar, value, percent in zip(bars, top_long_term["Amount"], top_long_term["share_percent"]):
    plt.text(value, bar.get_y() + bar.get_height() / 2, f" {value / 1e6:.2f}M ({percent:.1f}%)",
...
plt.title("HÌNH 2.4: BIÊN ĐỘ CHI TIÊU THEO BIẾN ĐỊNH TÍNH ÁP LỰC VÀ NỢ", fontsize=16, fontweight="bold")
...
plt.title("HÌNH 2.6: MẬT ĐỘ PHÂN PHỐI ĐIỂM TÍN NHIỆM TRONG QUẦN THỂ", fontsize=16, fontweight="bold")
...
plt.text(0.03, 0.78, f"Số người dưới ngưỡng: {below_count}\nTỷ lệ: {below_rate:.2f}%",
...
for value, color, label in [
    (0, "#1f77b4", "Không đạt mục tiêu tiết kiệm"),
    (1, "#ff7f0e", "Đạt mục tiêu tiết kiệm"),
]:
...
plt.plot(line_x, line_y, color="black", linewidth=2, label=f"Đường hồi quy | r = {correlation:.2f}")
...
plt.text(0.02, 0.05, "Diễn giải: thu nhập tăng kèm chi tiêu tăng\ncó thể cho thấy xu hướng vung tay chi tiêu.",
         transform=plt.gca().transAxes, bbox=dict(boxstyle="round", facecolor="white", alpha=0.85))
